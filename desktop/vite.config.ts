...
export default async function viteConfig(_config: ConfigEnv): Promise<UserConfig> {
  ...
  if (canResolveCommitName && process.env.FOLIA_COMMIT_NAME_LOOKUP === 'true') {
    try {
  ...
  const appVersionLabel = process.env.APP_VERSION_LABEL?.trim() || 'Musicloud development';
